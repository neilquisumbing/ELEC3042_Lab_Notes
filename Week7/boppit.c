...
uint8_t SPI_Read_Command(uint8_t reg) {
    ...
    PORTB &= ~_BV(2);  // S5 enabled (low)
    SPI_transfer(0x41);
    SPI_transfer(reg);
    data = SPI_transfer(0);
    PORTB |= _BV(2); // S5 disabled (high)
    return data;
}
...
void setup_SPI() {
    ...
    DDRB = 0b00101111;
    PORTB |= 0b00000100; // Set S5 signal high
    ...
    */
    ...
    /* Now that the SPI Interface is configured we need to send SPI commands to configure
     * the MCP23S17 port expander IC
     * 
     * We will configure PORTA as all outputs
     * PORTB 0-3 are OUTPUTS and 4-7 are INPUTS
     * We turn on pull-up resistors on PORTB
     */
    
    SPI_Send_Command(0x00, 0x00);   // Register IODIRA (PORTA data direction)
    SPI_Send_Command(0x01, 0xf0);   // Register IODIRB (PORTB data direction)
    SPI_Send_Command(0x0d, 0xff);   // Register GPPUB (PORTB GPIO Pullup)
}
